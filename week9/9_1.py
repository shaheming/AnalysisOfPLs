...
for i in range(0,len(words),WORDS_SIZE):
    if i + WORDS_SIZE < len(words):
        indices = [uniqs_indices[w] for w in words[i:i+WORDS_SIZE]]
    else:
        indices = [uniqs_indices[w] for w in words[i:-1]]
        # 7 [5,6]  5 - (7 - 5) = 3
        for _ in range(0,WORDS_SIZE - (len(words) - i)):
            indices.append(uniqs_indices[""])
    ...
    if i == 0:
        ...
        prediction = np.add(prediction,preds[-1][0])
