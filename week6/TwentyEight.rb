...
class DataStorageManager < Actor
  def dispatch(message)
    if message[0] == 'init'
      ...
      # forward
      send_message(@stop_word_manager, message)
    end
  end
  ...
  def process_words(message)
    ...
    @data.split.each do |w|
      send_message(@stop_word_manager,['filter',w])
    end
    send_message(@stop_word_manager,['top25',recipient])
  end
end
...
send_message(stop_word_manager, ['init', word_freq_manager])
threads<<stop_word_manager
storage_manager = DataStorageManager.new
send_message(storage_manager, ['init', ARGV[0], stop_word_manager])
